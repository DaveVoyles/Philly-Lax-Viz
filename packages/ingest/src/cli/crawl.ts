...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = { category: 'all' };
  for (const a of argv) {
...
function printHelp(): void {
  console.log(
    `Usage: tsx src/cli/crawl.ts [--max-pages=N] [--category=scoreboard|hs-summaries|rankings|all]
...
async function main(): Promise<void> {
  ...
  console.log(
    `[crawl] backend=${backend} cacheDir=${cacheDir} category=${args.category} maxPages=${args.maxPages ?? 'unlimited'}`,
  );
  ...
  const summaries: CrawlSummary[] = await crawlAll(
    categories,
    { maxPages: args.maxPages, ignoreWatermark: args.ignoreWatermark },
    {
  ...
  for (const s of summaries) {
    console.log(
      `  ${s.category}: pages=${s.pagesFetched} seen=${s.postsSeen} fetched=${s.postsFetched} cached=${s.postsAlreadyCached} skipped-girls=${s.postsSkippedGirls} stop=${s.stoppedReason}`,
    );
  }
