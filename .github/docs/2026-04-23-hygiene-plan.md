...
- **Lane 2 (Yoda):** Added `packages/ingest/src/scripts/reconcileTeamScores.ts` (review-queue only, never mutates `games`) + vitest. Scanned 562 games → **29 suspect team-game rows** written to `.github/docs/2026-04-23-team-score-reconcile-queue.json`. Regenerated cross-checks as `2026-04-23-cross-checks-v2.json` with resolution markers for game-154 Raggazino/Henzes/Hires (MaxPreps-confirmed score = 5) and `pending-review` triage on the rest. Notable anomalies surfaced: g446 WC Rustin per-player sum = 2× team score (likely double-attribution); g541 has BOTH sides flagged and team name `"Halftime – EA"` suggests a parser artifact. Commit: `299585761f74824f35673086e379c3b93c4a90db`.
...
- **Lane 3 (Leia):** Emitted `/.github/docs/2026-04-23-laxnumbers-aliases.csv` — 102 distinct LN unknown-team names across 160 anomalies, with top-3 fuzzy matches + 0.0–1.0 confidence per row, sorted by `anomaly_count DESC`. Reviewer fills `reviewer_decision` column (`accept1`/`accept2`/`accept3`/`reject`/`new`). No aliases auto-applied; pure read-only queue. Script: `packages/ingest/src/scripts/emitLaxNumbersAliasCsv.ts` (Node builtins only, no new deps). Open in Excel: `open -a "Microsoft Excel" .github/docs/2026-04-23-laxnumbers-aliases.csv`.
- **Lane 1 (Han 😉🚀):** Repointed stale ACR references `pllacr3087` → `adovizacr1771621563` in `docs/azure-deployment.md` + `.github/docs/2026-04-22-cleanup-and-laxnumbers-plan.md` (the three "expected" files — `Dockerfile`, `infra/azure-bootstrap.sh`, `.github/workflows/deploy.yml` — turned out to use GHCR, not ACR, so had no hits). Dockerfile `CMD` now prefers `/data/lacrosse.db` (Azure Files mount) over baked seed: `if [ -f /data/lacrosse.db ]; then cp /data/lacrosse.db "$DB_PATH"; elif [ -f /app/seed/lacrosse.db ] && [ ! -f "$DB_PATH" ]; then cp seed…; fi` — so future DB refreshes via Azure Files propagate on next ACA restart without an image rebuild. `ENV DB_PATH` and `azure-bootstrap.sh --env-vars` both set to `/tmp/lacrosse.db` (matching live ACA, preserves SMB-safety copy-to-ephemeral pattern). `pnpm -r test` 479/479 green. Changes landed in commit `7ff1396` (swept in by parallel Leia commit during concurrent lane execution; already on `origin/main`).
